...
const videoRoot = path.join(repo,'artifacts/video-raw');
...
const server = http.createServer((req,res) => {
  ...
  const file = path.resolve(root,url.pathname.slice(prefix.length) || 'bevy.html');
...
async function recordGameplay(page) {
  ...
  const output=fs.mkdtempSync(path.join(videoRoot,'session-'));
  ...
  try {
    ...
    fs.writeFileSync(path.join(videoRoot,'capture.json'),JSON.stringify(report,null,2));
...
if(require.main===module)(async()=>{
  ...
  try {
    ...
    await page.goto('http://127.0.0.1:'+server.address().port+prefix+'bevy.html',{waitUntil:'domcontentloaded'});
