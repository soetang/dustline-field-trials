...
(async () => {
  ...
  const serverRoot=exported?path.join(repo,'_site'):repo;
  const prefix=exported?'/dustline-field-trials/':'/';
  const entry=exported?'bevy.html':'bevy/bevy.html';
  ...
  const server = http.createServer((req, res) => {
    ...
    const file = path.resolve(serverRoot, relative ? relative + (relative.endsWith('/') ? 'index.html' : '') : entry);
  ...
  const gameURL='http://127.0.0.1:'+server.address().port+prefix+entry;
  let browser, page;
  const recentLogs = [];
  let lastState = null;
  ...
  try {
    ...
    if (process.env.CAPTURE) await page.screenshot({ path: path.join(artifacts, 'dustline-menu.png'), timeout: 30000 });
    ...
    if (process.env.CAPTURE) await page.screenshot({ path: path.join(artifacts, 'dustline-expanded-spawn.png'), timeout: 30000 });
    ...
    if (process.argv.includes('--capture-spawn')) {
      ...
      await page.screenshot({path: path.join(artifacts, 'dustline-operators-spawn.png'), timeout: 20000});
    ...
    if (process.env.CAPTURE) await page.screenshot({ path: path.join(artifacts, 'dustline-armory.png'), timeout: 30000 });
    ...
    if (process.env.CAPTURE) await page.screenshot({ path: path.join(artifacts, 'dustline-gameplay.png'), timeout: 30000 });
    ...
    if (process.argv.includes('--playtest')) {
      ...
      fs.writeFileSync(path.join(artifacts, 'browser-playtest.json'), JSON.stringify(report, null, 2));
    ...
    if (process.env.CAPTURE) await page.screenshot({ path: path.join(artifacts, 'dustline-pause-small.png'), timeout: 30000 });
    ...
    if (page) {
      ...
      if (process.env.CAPTURE) await page.screenshot({ path: path.join(artifacts, 'dustline-test-failure.png'), timeout: 5000 }).catch(() => {});
    }
    throw error;
  } finally {
