...
(async () => {
  ...
  assert.equal(input().compactHud,true,'Advertise support before the engine omits static metadata');
  ...
  const livePacket={...state,ammo:27,yaw:state.yaw+.2};
  for(const field of ['map','spawn','attackerSpawn','sites','layoutScale'])delete livePacket[field];
  api.render(livePacket);
  const complete=api.getState();
  assert.equal(complete.ammo,27,'Live values must still update without layout metadata');
  for(const field of ['map','spawn','attackerSpawn','sites','layoutScale'])assert.deepEqual(complete[field],state[field]);
  assert.equal(createdCanvases.length,2,'Metadata-free updates reuse both cached map layers');
  assert.ok(!('map' in livePacket),'Rendering must not mutate the incoming engine packet');
  complete.map[0][0]=99;assert.notEqual(api.getState().map[0][0],99,'Feedback snapshots cannot mutate cached terrain');
  assert.ok(JSON.stringify(livePacket).length<JSON.stringify(state).length*.5,'Steady HUD packets should lose at least half their JSON bytes');
  const differentMap=structuredClone(state);differentMap.map[0][0]=differentMap.map[0][0]===1?0:1;
  api.render(differentMap);assert.equal(createdCanvases.length,4,'Changed layout must invalidate both briefing and radar backgrounds');
  nodes.get('radar').width++;api.render(differentMap);assert.equal(createdCanvases.length,5,'Changed canvas size must invalidate the background');
  ...
  assert.equal(nodes.get('round-cue-time').textContent, '7');
  assert.match(nodes.get('round-cue-detail').textContent, /Movement locked/);
  api.render({...state,phase:'live',time:89});
  assert.equal(nodes.get('round-cue-label').textContent,'ROUND LIVE');
  assert.equal(nodes.get('round-cue-time').textContent,'GO');
  api.render({...state,phase:'live',time:85});assert.equal(nodes.get('round-announcement').hidden,true);
  api.render(state);
