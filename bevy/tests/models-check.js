...
const models = path.resolve(__dirname, '../assets/models');
for (const team of ['ct', 't']) {
  const data = fs.readFileSync(path.join(models, `${team}_operator.glb`));
...
for (const weapon of ['m4','ak','awp','deagle']) {
  const data=fs.readFileSync(path.join(models,`view_${weapon}.glb`));
