...
try {
  if (location.protocol === 'file:') throw new Error('Open http://localhost:8765/bevy/bevy.html after running npm run serve from the repository root');
  const response = await fetch('./web/current.json', { cache: 'no-store' });
  if (!response.ok) throw new Error(`The game release is missing (HTTP ${response.status}). Run npm run build:bevy`);
