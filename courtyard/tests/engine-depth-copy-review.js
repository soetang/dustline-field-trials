...
function validateEngine(review, templates, mode) {
  ...
  assert.equal(review.depth_copy_review, true);
...
function run(manifestPath) {
  ...
  for (const mode of ['baseline', 'patched']) {
    ...
    const args = [path.join(__dirname, 'map-review-browser.js'), '--depth-copy-review', '--capture', `--engine-template=${template}`];
    ...
    validateEngine(JSON.parse(fs.readFileSync(path.join(directory, 'captures.json'), 'utf8')), templates, mode);
  }
  ...
  const comparison = require('./depth-copy-review').compareReviews(readReview('baseline'), readReview('patched'), readImage('baseline'), readImage('patched'));
...
if (require.main === module) {
  try {
    assert.equal(process.argv.length, 3, 'Usage: engine-depth-copy-review.js VERIFIED_TEMPLATES_JSON');
