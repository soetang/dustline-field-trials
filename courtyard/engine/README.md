...
```sh
node courtyard/tests/backbuffer-gl-audit-check.js
node courtyard/tests/engine-backbuffer-check.js --source /absolute/path/to/godot-4.7.2-stable
node courtyard/tests/map-review-browser.js --engine-lifecycle --capture --engine-template=/absolute/path/to/baseline.zip
node courtyard/tests/map-review-browser.js --engine-lifecycle --expect-cached-backbuffer --presentation-cache --capture --engine-template=/absolute/path/to/patched.zip
```
