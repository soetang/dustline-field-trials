...
assert.ok(input,'Usage: node tests/video-check.js path/to/gameplay.webm');
assert.ok(fs.statSync(input).size<12*1024*1024,'Public video must stay below 12 MiB');
const before=fs.statSync(input);
fs.mkdirSync('artifacts/video-analysis',{recursive:true});
const output=fs.mkdtempSync('artifacts/video-analysis/frames-');
