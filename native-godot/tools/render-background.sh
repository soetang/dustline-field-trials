...
DISPLAY=":$display_number" WAYLAND_DISPLAY=dustline-no-host-display LIBGL_ALWAYS_SOFTWARE=1 \
  "$godot_bin" --path "$project_dir" --display-driver x11 --rendering-method gl_compatibility \
  --script res://tests/operator_gallery.gd -- --test "$@" 2>&1 | tee "$log_dir/renderer.log"
if rg -q 'SCRIPT ERROR:|^ERROR:|falling back' "$log_dir/renderer.log"; then exit 1; fi
rg -q '^OPERATOR_GALLERY_OK' "$log_dir/renderer.log"
