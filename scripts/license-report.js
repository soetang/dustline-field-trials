...
process.stdin.on('data', chunk => input += chunk).on('end', async () => {
  ...
  for (const pkg of packages) {
    ...
    if (!notices) {
      const saved = `licenses/upstream/${pkg.name}-${pkg.version}.txt`;
      if (!fs.existsSync(saved) && process.argv.includes('--fetch-missing')) {
        ...
        if (found.some(Boolean)) {
          fs.mkdirSync('licenses/upstream',{recursive:true});
  ...
  fs.writeFileSync('licenses/rust-dependencies.txt',output);
